...
::: fragment
- [Scales]{style="color: orange;"} defines *how* the information (data)
  is mapped to [aesthetic attributes]{style="color: red;"} work
- [Guides]{style="color: green;"} defines how
  [scales]{style="color: orange;"} are represented
:::

## Scales and guides {.scrollable}

- **Scales** controls the limits and ticks of the x-axis and y-axis
- **Guides** can roughly be understood as similar to **figure legends**,
  it's where we read data values from the graph

::: fragment
Which components are controlled by **scales**? And which are controlled
by **guides**?

![](figures/viz/scale-guides.png)
:::
...
::::: columns
::: {.column width="50%"}
- Some other types of scales: colour, point shape, line type, point size
:::

::: {.column width="50%" style="text-align: center;"}
![](figures/viz/ColorGraph.png){style="display: block; margin: 0 auto;"}
:::
:::::
::::::
...
ggplot(data = df, mapping = aes(x = coverage, y = inequality)) +
  geom_point(mapping = aes(size = pop, color = region)) +
  geom_text(mapping = aes(label = country)) +
  scale_color_manual(values = my_pallete, guide = guide_legend(order = 2)) +
...
| `geom_*`          | description                     | Parameters      |
+===================+=================================+=================+
| `geom _boxplot()` | Create a boxplot from a         | Required        |
