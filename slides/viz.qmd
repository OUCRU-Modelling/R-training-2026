...
- Mapping from information (data) to [aesthetic attributes]{.fragment
...
- [Geometric objects]{.fragment .highlight-blue fragment-index="1"} are
  what you see on the plot (points, lines, bars)
...
::: fragment
- [Scales]{style="color: orange;"} defines the range and values of an
  [aesthetic]{style="color: red;"}
:::
...
- Each **bar in a histogram** can be a "sum" or a
  "percentage/frequency", depending on the (optional) statistical
...
**Note**: statistical transformation on the scales [do
not]{style="color: red;"} change the data, just how we "observe" it
...
+====================+===========================+========================+
| `geom_line()`      | Connect observations in   | Required: `x`, `y`     |
|                    | order of the x-axis       |                        |
+--------------------+---------------------------+------------------------+
| `geom_histogram()` | Divide continuous `x`     | Required: `x`, and     |
|                    | into bins and count       | `bins` or `binwidth`   |
|                    | observations.             |                        |
+--------------------+---------------------------+------------------------+
| `geom_col()`       | Bar chart using provided  | Required: `x`, `y`     |
|                    | `x` and `y` values.       |                        |
+--------------------+---------------------------+------------------------+
| `geom_bar()`       | Bar chart with one axis   | Required: `x` or `y`,  |
|                    | computed using `stat`.    | `stat` (default:       |
|                    |                           | `"count"`)             |
+--------------------+---------------------------+------------------------+

More at [`ggplot2` package reference](https://ggplot2.tidyverse.org/reference/index.html#geoms)
:::
...
|                 | regression     |                                                    |
|                 | line plot from | - `method`: select regression model                |
|                 | `x` and `y`    |   (`"auto"`, `" lm"`, `"glm"`, `"gam"`, `"loess"`) |
|                 |                |                                                    |
...
| `geom_*`          | description                         | Parameters   |
+===================+=====================================+==============+
| `geom _boxplot()` | Create a boxplot from a continuous  | Required     |
|                   | variable.                           | aesthetic:   |
|                   |                                     | `x`          |
...
- Vector format (pdf, eps, wmf, emf, svg)
  - digital image consisting of independent geometric objects (segments,
    polygons, curves, etc.)
...
- Not about:
  - [graph type](https://www.data-to-viz.com/) ($\approx$ language,
    words)
  - font size, background colour ... Some of these are specified in a
