...
author: "Hung Tran and Thinh Ong"
format:
  revealjs:
    slide-number: true
...
::: {.v-center-container style="display: flex; height: 80%; align-items: center; justify-content: center;"}
...
[1]: figures/qt/qt-website2.png
[2]: https://oucru-biostats.github.io/Introduction-to-Medical-Statistics/ "Let's try it out"



## Presentation

<iframe src="viz.html" width="100%" height="500px" style="border:none;"></iframe>

## Report/Manuscript

<iframe src="../handouts/report.html" width="100%" height="500px" style="border:none;"></iframe>
...
#| fig-width: 2
#| fig-height: 2
# Run install.packages("qrcode") in R console if not installed
library(qrcode)
qr_code("https://quarto.org/docs/gallery/") |> plot()

```
Or click [here](https://quarto.org/docs/gallery/){target="_blank"}
:::
:::::
...
[3]:  figures/qt/qt-search-2.png
[4]:  https://quarto.org/ "Let's try it out"

# Let's write a manuscript

## Creating an Rproject file
1. Download the in-class example: [Download my_project.zip](/handouts/my_project.zip){.btn .btn-primary target="_blank" role="button"}
2. Extract the downloaded zip folder.
3. Open RStudio
4. Go to `File > New Project... > Existing Directory`, browse to the extracted `my_project` folder, and click `Create Project` to generate `covid.Rproj` inside it.
...
The simple regression model is $Y_i = \beta_0 + \beta_1 X_i + \epsilon_i$ `$Y_i = \beta_0 + \beta_1 X_i + \epsilon_i$`

Display math: 

The Gaussian probability density function is:
$$f(x) = \frac{1}{\sqrt{2\pi\sigma^2}} e^{-\frac{(x - \mu)^2}{2\sigma^2}}$$ `$$f(x) = \frac{1}{\sqrt{2\pi\sigma^2}} e^{-\frac{(x - \mu)^2}{2\sigma^2}}$$`
...
<iframe class="slide-deck" src="qt-example.html" style="width: 100%; height: 484px; border: none;"></iframe>
:::
::::
:::::::
...
[Blooket Set](https://dashboard.blooket.com/set/67d14a2bd40d72e0b5cfc37d){target="_blank"}
...
-   [Click here](https://github.com/quarto-journals/){target="_blank"} to see a list of available journal templates in Quarto
...
## Use our template
- Open `day5_manuscript.qmd` from the `templates/` folder inside `my_project`.
- Copy all the contents inside `day5_manuscript.qmd` and paste to the `covid.qmd`.
...
First let's load the packages and have a look at the dataset [simulated_covid.rds](https://github.com/OUCRU-Modelling/R-training-2025/raw/refs/heads/main/slides/data/simulated_covid.rds).
...
## Reuse code with new data {.smaller}
...
<iframe class="slide-deck" src="qt-example-slide.html" style="width: 100%; height: 500px; border: 1px solid #ccc; box-sizing: border-box;"></iframe>
...
- [Click to view this exercise](/handouts/exercises_Day5.qmd){.btn .btn-outline-primary target="_blank" role="button"}
