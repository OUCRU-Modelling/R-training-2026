...
- Create an Rproject file in side the folder my_project
- Open that Rproject
...
## Add text, math formula, symbol
Quarto uses LaTeX syntax to render equations seamlessly:
Inline math: Wrap LaTeX code with single dollar signs $ LaTex $
Display math: Wrap LaTeX code with double dollar signs $$ LaTex $$
...
toc: true
---
Or we can include some math formula here: $4+4=8$ (alternatively, a more complex formula as math display via visual/insert).

The simple regression model is $Y_i = \beta_0 + \beta_1 X_i + \epsilon_i$.
