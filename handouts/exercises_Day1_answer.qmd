...
**Answer**: The variable `dob` has a strange format. The variable `survived` has
values 0 and 1, which are not very informative.  
...
**Answer**: 1309  rows and 17 colums
...
`survived` is of mode numeric, while one would expect it to be a categorical variable
of mode character.
...
Many are of class and mode character, and the `summary` function does not give any
information. The `dob` variable has negative values.
...
**Answer**: females were much more likely to survive than males, especially among
those who travelled first class
...
**Answer**: Passenger class and sex are summarized properly, and status is summarized
